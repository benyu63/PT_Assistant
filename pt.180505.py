...
PT_Sites = (    #  pt站信息，没加注释的字段最好不要改
    {
        ...
        'headers': {
            ...
            #'Cookie': '',
            'Cookie': '__cfduid=d3eb963caf35f1684c95f30515d6181831525000074; c_secure_uid=MTgxNw%3D%3D; c_secure_pass=673725b3a35d5abc2a7915c431b2d150; c_secure_ssl=eWVhaA%3D%3D; c_secure_tracker_ssl=eWVhaA%3D%3D; c_secure_login=bm9wZQ%3D%3D',
    ...
    },                          #  后面的注释懒得一个一个加了，跟这个一样，自己看吧
    {
        ...
        'headers': {
            ...
            #'Cookie': '',
            'Cookie': 'Hm_lvt_4999ad46e9bd1069ab4b3202d2987d41=1525279768; _ga=GA1.2.1875184634.1525279768; _gid=GA1.2.408609894.1525279768; _gat=1; c_secure_uid=MzMwNDk%3D; c_secure_pass=9ad2acf8e517f23ed2ec6a0f0dd5bd3e; c_secure_ssl=eWVhaA%3D%3D; c_secure_tracker_ssl=eWVhaA%3D%3D; c_secure_login=bm9wZQ%3D%3D; Hm_lpvt_4999ad46e9bd1069ab4b3202d2987d41=1525279795',
            'Host': 'pt.gztown.net',
    ...
    },
    {
        ...
        'headers': {
            ...
            'Connection': 'keep-alive',
            #'Cookie': '',
            'Cookie': '__cfduid=d107f3989bd195b78a6f66d0d44b3bed71525004742; c_secure_uid=ODk1NTU%3D; c_secure_pass=dba7ba6846c15ab310ffa57c55ad946a; c_secure_ssl=bm9wZQ%3D%3D; c_secure_tracker_ssl=bm9wZQ%3D%3D; c_secure_login=bm9wZQ%3D%3D',
            'Host': 'hdhome.org',
    ...
    },
    {
        ...
        'headers': {
            ...
            'Connection': 'keep-alive',
            #'Cookie': '',
            'Cookie': '__cfduid=d467d6436b63021000a6d78663ea3c31d1525349360; c_secure_uid=NDI1MTI%3D; c_secure_pass=057e78bd585d48a5c22ee3e092a1afdc; c_secure_ssl=eWVhaA%3D%3D; c_secure_tracker_ssl=bm9wZQ%3D%3D; c_secure_login=bm9wZQ%3D%3D',
            'Host': 'ourbits.club',
...
def WriteLog( s, b=1 ):
    if ( b == 0 ):  #  找到种子就写入log文件，否则输出到屏幕，避免log文件过大降低性能
        if ( os.path.sep == '\\' ):
            f = open( log_filename, 'a', encoding='gbk' )
        else:
...
try:                        #  这个日志文件过大或者出现问题，可以直接删除，不影响程序运行
    if ( os.path.sep == '\\' ):
        log_filename = re.sub( r'\\[^\\]+$', '', os.path.abspath(__file__) ) + r'\pt.log'
        f = open( log_filename, 'r', encoding='gbk' )
    else:                   #  Windows目录分隔符跟Linux不同，必须分别处理
        log_filename = re.sub( r'/[^/]+$', '', os.path.abspath(__file__) ) + r'/pt.log'
        f = open( log_filename, 'r', encoding='utf-8' )
    log_content = f.read()  #  读入日志文件，其中保存有以前筛选出的种子信息，避免筛出重复种子
    f.close()
except:
    log_content = ''
...
for pt_site in PT_Sites:                #  根据上面的数据结构循环遍历各pt站
    ...
    if ( ( b != 3 ) and ( b != 5 ) ):   #  处理非正常页面
        WriteLog( url + ' wrong page, dump html data below, check the url and the cookies plz' )
        WriteLog( content )          #  此函数参数的用法:0写入日志文件，1不写文件只输出到屏幕
        WriteLog( str(headers) )
        continue
    if ( b == 3 ):  #  需要签到
        ...
        if ( matcher != None ):  #  签到成功
            WriteLog( pt_site['address'] + ' ' + matcher.group( 0 ) + '魔力', 0 )  #  这个要写文件
            url = pt_site['address'] + pt_site['torrents']
            headers = pt_site['headers']
            encoding = pt_site['encoding']
            html = GetHTML( url, headers, encoding )  #  重新读取种子页面
        else:
            WriteLog( url + ' something might be wrong, \'ll sign next round' )
            WriteLog( content )
            continue
    ...
    for i in range(100):  #  前100个种子，本来这个循环50就够，谁知道ourbits置顶那么多...
        ...
        try:
            ...
            WriteLog( torrenturl )  
            continue
    if ( content == '' ):  #  没筛到种子也要说一下，不然看日志以为程序死了
        WriteLog( pt_site['address'] + ' nothing found.' )
    mail_content += content  #  各个站提取的种子累加到这个变量里
if ( mail_content != '' ):  #  发邮件
    WriteLog( 'found torrents:' + mail_content, 0 )
    '''
    mail_host = ''  #  邮箱smtp服务器地址
    mail_user = ''  #  邮箱用户名
    mail_pass = ''  #  邮箱密码
    '''
    mail_host = 'smtp.qq.com'       #  邮箱smtp服务器地址
    mail_user = '714447@qq.com'     #  邮箱用户名
    mail_pass = '4wo3Ai4cao2liu4'   #  邮箱密码
